...
export const ExecutiveFutureReadyReport: React.FC<ExecutiveFutureReadyReportProps> = ({
  ...
  return (
    <div className={`w-full bg-white text-slate-900 ${isPrintOnly ? "p-4" : "p-6 rounded-3xl border border-slate-200 shadow-xl"}`}>
        ...
        <div className="no-print flex items-center justify-between pb-4 mb-4 border-b border-slate-200">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold flex items-center space-x-1.5 shadow-sm cursor-pointer"
            >
      ...
      <div className="flex items-start justify-between pb-3 border-b-2 border-slate-900">
        <div className="flex items-center space-x-3">
          ...
          />
          <div>
            <h1 className="text-lg font-display font-black tracking-tight text-slate-900 uppercase">
              Lab of Future • Future-Ready Talent & Career Trajectories
            </h1>
            <div className="text-[11px] font-semibold text-slate-600 flex items-center space-x-2 mt-0.5">
        ...
        <div className="text-right text-[10px] font-mono text-slate-500">
          ...
          <div className="font-bold text-slate-700 mt-0.5">Generated: {new Date().toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2.5 my-3">
        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200">
          <div className="text-[9px] uppercase font-bold text-slate-400 font-mono">Talent Headcount</div>
          <div className="text-base font-display font-black text-slate-900 mt-0.5">
            {totalCandidates} Candidates
          </div>
        </div>

        <div className="p-2.5 rounded-xl bg-blue-50/60 border border-blue-200">
          <div className="text-[9px] uppercase font-bold text-blue-600 font-mono">Career Trajectories</div>
          <div className="text-base font-display font-black text-blue-900 mt-0.5">
            {distinctRoles} Industry Tracks
          </div>
        </div>

        <div className="p-2.5 rounded-xl bg-emerald-50/60 border border-emerald-200">
          <div className="text-[9px] uppercase font-bold text-emerald-700 font-mono">Average FR Score</div>
          <div className="text-base font-display font-black text-emerald-800 mt-0.5">
            {avgScore} / 100
          </div>
        </div>

        <div className="p-2.5 rounded-xl bg-amber-50/60 border border-amber-200">
          <div className="text-[9px] uppercase font-bold text-amber-700 font-mono">Market Salary Range</div>
          <div className="text-xs font-display font-black text-amber-900 mt-1">
            ${Math.round(minSalary / 1000)}K – ${Math.round(maxSalary / 1000)}K / yr
          </div>
        </div>

        <div className="p-2.5 rounded-xl bg-slate-900 text-white">
          <div className="text-[9px] uppercase font-bold text-cyan-400 font-mono">Key Corporate Ecosystem</div>
          <div className="text-[10px] font-bold text-slate-200 mt-1 truncate">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse border border-slate-200 text-[10px]">
          <thead>
            <tr className="bg-slate-100 text-slate-700 uppercase font-mono font-bold text-[9px] border-b border-slate-300">
              <th className="py-2 px-2.5 border-r border-slate-200 w-12 text-center">Rank</th>
              <th className="py-2 px-2.5 border-r border-slate-200 w-36">Student & Centre</th>
              <th className="py-2 px-2.5 border-r border-slate-200 w-24 text-center">Future Ready</th>
              <th className="py-2 px-2.5 border-r border-slate-200">Target Career Role & Sector</th>
              <th className="py-2 px-2.5 border-r border-slate-200 w-36">Tentative Market Salary</th>
              <th className="py-2 px-2.5 border-r border-slate-200 w-44">Demanding Industry Giants</th>
              <th className="py-2 px-2.5 w-36">Key Competency Triad</th>
            </tr>
          </thead>
            ...
            {enrichedStudents.map((item, idx) => {
              ...
              return (
                <tr key={student.id} className={idx % 2 === 1 ? "bg-slate-50/50" : "bg-white"}>
                  {/* Rank */}
                  <td className="py-2 px-2 border-r border-slate-200 text-center font-display font-bold">
                  ...
                  <td className="py-2 px-2.5 border-r border-slate-200">
                    ...
                    <div className="text-[9px] text-slate-500">{student.centre.city} • {student.domain}</div>
                  </td>
                  ...
                  <td className="py-2 px-2.5 border-r border-slate-200 text-center">
                    ...
                    <div className="text-[8px] text-slate-400 font-mono">
                      ST:{breakdown.streamerAvg} | 21:{breakdown.twentyFirstCenturyAvg}
                    </div>
                  </td>

                  {/* Target Career Role */}
                  <td className="py-2 px-2.5 border-r border-slate-200">
                    <div className="font-bold text-slate-900">{careerRole.roleTitle}</div>
                  ...
                  <td className="py-2 px-2.5 border-r border-slate-200">
                    <div className="font-display font-bold text-emerald-800">
                  ...
                  <td className="py-2 px-2.5 border-r border-slate-200">
                    <div className="flex flex-wrap gap-1">
                      {careerRole.hiringOrganizations.slice(0, 4).map(org => (
                        <span key={org} className="px-1.5 py-0.2 rounded bg-slate-100 border border-slate-200 font-bold text-[8px] text-slate-800">
                          {org}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="py-2 px-2.5">
                    <div className="font-semibold text-slate-800">{careerRole.competencyTriplet}</div>
                    <div className="text-[8px] text-emerald-700 font-mono font-bold mt-0.5">
      ...
      <div className="mt-3 pt-2 border-t border-slate-200 flex items-center justify-between text-[9px] text-slate-500">
        ...
        <div className="font-mono">
          Official Credential • Confidential Corporate Talent Dossier
        </div>
      </div>
    </div>
  );
